...
## Идея в работу: ежемесячный прирост комбинаций

Комбинации в `comboGroups` это единственный по-настоящему уникальный контент на
сайте: одиночный `♥` одинаков у всех, а составленная строка принадлежит нам.
Сейчас их 370 на 15 категориях, у конкурента порядка 100-200 на страницу.

План: раз в месяц догенерировать по 5-10 строк на категорию через Claude и
дописать в каталог. Ценность в росте библиотеки, а не в свежести: символы не тот
запрос, где Google премирует обновления, поэтому менять ради обновления смысла
нет.

Что обязательно нужно сделать до записи в каталог, иначе накопится мусор:

- проверка покрытия шрифтом, логика `canRender` уже есть в `scripts/generate-og.mjs`
- отсев одиночных комбинирующих знаков, они рендерятся пустотой
- сверка на дубли со всеми существующими строками, как в `scripts/dedupe-combos.mjs`
- контроль длины: лимит био 80 символов в TikTok и 150 в Instagram

Шесть категорий пропускаем сознательно: math, currency, special-characters,
keyboard, icon-copy-paste, symbols-for-usernames. Декоративная композиция из
знаков валют не отвечает ни на один реальный запрос.
